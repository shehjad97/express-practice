...
const getMembers = catchAsync(async (req, res) => {
    const members = await MemberModel.find({}, { name: true, address: true });
    return apiResponse(res, httpStatus.OK, { data: members });
});

const getMember = catchAsync(async (req, res) => {
    const member = await MemberModel.findOne({ _id: req.params._id }, { name: true, address: true });
...
const addMember = catchAsync(async (req, res) => {
    ...
    const newMember = new MemberModel({ name, address });
...
const updateMember = catchAsync(async (req, res) => {
    const { name, address } = req.body;
    const update = await MemberModel.updateOne({ _id: req.params._id }, { name, address });
